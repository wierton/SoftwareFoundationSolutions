...
Fixpoint split {X Y : Type} (l : list (X*Y))
   ...
   | cons e l' =>
       match e with
       | (x, y) => (cons x (fst (split l')), cons y (snd (split l')))
       end
   end.
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter evenb (filter (fun n:nat => (ltb 7 n)) l).
...
Definition partition {X : Type}
                     ...
                     (l : list X)
                   : list X * list X :=
  (filter test l, filter (fun x:X =>(notb (test x))) l).
...
(** Prove the correctness of [fold_length].  (Hint: It may help to
    ...
    goal.) *)
Theorem fold_length_e_l:
  forall X (e:X) (l: list X), fold_length (e :: l) = S (fold_length l).
Proof.
  constructor.
Qed.
...
Definition fold_map {X Y: Type} (f: X -> Y) (l: list X) : list Y
  := @fold X (list Y)
  (fun (e:X) (l':list Y) => (f e) :: l') l (@nil Y) .
